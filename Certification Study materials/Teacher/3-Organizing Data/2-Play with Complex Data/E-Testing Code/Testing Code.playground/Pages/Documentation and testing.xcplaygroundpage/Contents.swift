/*:
 ...
 You'll use comments to document your code in Swift. If you use the special comment format `///`, you'll get a bonus: Swift will automatically create and format documentation that you can access by Option-clicking the item—just as you do for internal types. Try it with the `PennyBank` type and its methods below.
...
/// - Note: Penny banks can hold a maximum of 255 pennies. Negative balances are not permitted.
class PennyBank {
...
/*:
 ...
 What are the limits of the `deposit` and `withdraw` methods? Make some test cases below by creating instances of `PennyBank` and calling the two methods with values that should produce valid results—as well as some tests that should cause it to crash. Verify valid results by comparing the method call to the expected output as demonstrated. (You'll have to comment out the tests that crash your code in order to have them alongside other tests.)
 */
